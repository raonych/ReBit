...
export default function DropDownCategorias() {
  ...
  return (
    <Menu as="div" className="relative inline-block text-left">
      ...
      <MenuItems
        transition
        className="absolute right-0 z-80 mt-2 w-56 origin-top-right divide-y divide-gray-100 rounded-md bg-white shadow-lg ring-1 ring-black/5 transition focus:outline-hidden data-closed:scale-95 data-closed:transform data-closed:opacity-0 data-enter:duration-100 data-enter:ease-out data-leave:duration-75 data-leave:ease-in"
      >
        {
          categorias.map((categoria) => (
            <div key={categoria.id} className="py-1">
              <MenuItem>
                <Link
                  href={"/produtos?categoria="+categoria.nome}
                  className="block px-4 py-2 z-80 text-sm text-gray-700 data-[focus]:bg-gray-100 data-[focus]:text-gray-900 data-[focus]:outline-none"
                >
                  {categoria.nome}
                </Link>
              </MenuItem>
